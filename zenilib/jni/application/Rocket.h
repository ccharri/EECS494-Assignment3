#ifndef __game__Rocket_h__
#define __game__Rocket_h__

#include <zenilib.h>

#include <memory>

#include "Game_Object.h"
#include "Model_Wrapper.h"

class Tower_Weapon;
class Enemy;

class Rocket : public Game_Object {
public:
	Rocket( std::weak_ptr<Tower_Weapon> owner_, std::weak_ptr<Enemy> target_, float damage_, float speed_, Zeni::Point3f position_, Zeni::Vector3f size_ = Zeni::Vector3f(10,2,2), Zeni::Quaternion facing_ = Zeni::Quaternion(), Zeni::Vector3f scale_ = Zeni::Vector3f(1,1,1));

	virtual void on_logic( float time_step );

	virtual std::shared_ptr<Zeni::Model> getModel() const override {return model.getModel();};

	float getSpeed() const {return speed;}

private:
	static Model_Wrapper model;

	Zeni::Collision::Capsule collider;

	std::weak_ptr<Tower_Weapon> owner;

	float damage;
	float speed;
	std::weak_ptr<Enemy> target;

	void updateCollider() override;
	void explode();
};

#endif